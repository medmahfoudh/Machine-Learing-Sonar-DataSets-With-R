...
#Pour simplifier notre travail, nous pouvons créer les data frame suivants
...
# Entraînement d'un modèle sur des données
...
#Pour calculer la précision, nous additionnons toutes les observations correctement classées (situées 
#dans la diagonale) et les divisons par le nombre total de classes
...
#n pour calculer la précision des étiquettes d'apprentissage y_train et 
#des prédictions validées par recoupement knn_loocv
...
SEED <- 2016
set.seed(SEED) 
# Créer L'entrainement au 70% de datasets. 
